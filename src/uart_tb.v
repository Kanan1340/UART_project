...
localparam integer SYS_CLK_NS   = 10;
...
wire uart_clk_out;
wire frame_errorH;
assign uart_clk_out = dut.uart_clk;
uart_top #(
    .SYS_CLK_FREQ(XTAL),
    .BAUD_RATE(BAUD)
) dut (
...
task automatic reset_and_initialize();
begin
    sys_rst_l      = 1'b0;
    xmit_H         = 1'b0;
    xmit_dataH     = 8'h00;
...
initial begin
    ...
    check_bus("rec_data_no_stop", rec_dataH, snap_rec,
              "rec_dataH unchanged after missing stop");
    ...
    check_bus("false_start_bit", rec_dataH, snap_rec,
              "rec_dataH unchanged after glitch");
    ...
    repeat(200) @(posedge sys_clk);
    check_bit("reset_in_start", xmit_active, 1'b0, "xmit_active==0 after rst");
    check_bit("reset_in_start", uart_XMIT_dataH, 1'b1, "line idle after rst");
    repeat(200) @(posedge sys_clk);


    $display("\n--- FID 12: reset_during_data ---");
    reset_and_initialize();
    ...
    repeat(200) @(posedge sys_clk);
    check_bit("reset_in_data", xmit_active, 1'b0, "xmit_active==0 after rst");
    check_bit("reset_in_data", uart_XMIT_dataH, 1'b1, "line idle after rst");
    repeat(200) @(posedge sys_clk);
